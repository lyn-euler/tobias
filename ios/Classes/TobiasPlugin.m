...
- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    } else if ([@"deduct" isEqualToString:call.method]) {
        [self _deduct:call result:result];
    } else {
...
- (BOOL)handleOpenURL:(NSURL *)url {
    ...
    if ([url.host isEqualToString:@"safepay"]) {
        ...
        [[AlipaySDK defaultService] processAuth_V2Result:url standbyCallback:^(NSDictionary *resultDic) {
            [weakSelf onAuthResultReceived:resultDic];
        }];

        return YES;
    }else if ([url.host isEqualToString:@"apmqpdispatch"]) {
        [AFServiceCenter handleResponseURL:url withCompletion:^(AFServiceResponse *response) {
            [weakSelf handleServiceResult:response.result];
        }];
        return YES;
    }
    return NO;
...
- (void)_deduct:(FlutterMethodCall *)call result:(FlutterResult)result {
    NSString *urlScheme = [self fetchUrlScheme];
    if (!urlScheme) {
        result([FlutterError errorWithCode:@"AliPay UrlScheme Not Found" message:@"Config AliPay First" details:nil]);
        return;
    ...
    self.callback = result;
    [AFServiceCenter callService:AFServiceDeduct withParams:@{
         kAFServiceOptionBizParams: @{
             @"sign_params": call.arguments[@"signParams"] ? : @""
         },
         kAFServiceOptionCallbackScheme: urlScheme
     } andCompletion:^(AFServiceResponse *response) {
        [self handleServiceResult:response.result];
    }];
}
